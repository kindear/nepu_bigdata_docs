...
- `document_id` 不重复、不为空
- 顶层 `source_url` 为网页完整网址；`attachments` 内的 `source_url` 为附件原始下载地址，入库时对应 `attachment.source_url` 列
- `attachments` 无附件时为空数组 `[]`
- 附件中 `document_id` 与网页记录一致
- 不写入本地文件路径
- 附件 `object_key` 在采集阶段为 `null`，上传 S3 后回填实际路径
...
文档解析结果按 4.1 规范保存为独立 JSON 文件，每份网页或附件对应一个文件，便于按 `document_id` 或 `attachment_id` 直接读取。

保存位置：

```text
parsed/web/doc_0001.json              # 网页解析结果
parsed/attachments/att_0001.json      # 附件解析结果
```

> 4.1 也支持保存为 JSONL（如 `parsed/documents.jsonl`），适合批量处理场景。实际项目中选其一即可，JSONL 在分块阶段（4.2）逐行读取同样方便。
