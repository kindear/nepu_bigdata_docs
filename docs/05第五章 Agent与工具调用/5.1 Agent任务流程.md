...
> **拓展：使用 LangChain Agent 框架**：上述 (七)~(九) 的手写 Agent 流程（任务分类 → 工具选择 → 参数校验 → 工具执行），LangChain 也提供了更自动化的方案。如果已在 5.2 中使用 `@tool` 装饰器定义工具，可以配合 LangChain 的 Agent 框架简化调用：
>
> 安装依赖：
>
> ```bash
...
>
> # tools 来自 5.2（九）中 @tool 装饰后的函数列表
...
> ```
>
> 其中 `llm` 为 4.5/6.1 中配置的 `ChatDeepSeek` 实例。`create_react_agent` 会自动完成：
> - 判断是否需要调用工具；
> - 选择正确的工具和参数；
> - 执行工具并获取结果；
> - 将工具结果与用户问题整合后调用 LLM 生成最终回答。
>
> **建议**：课程基础项目推荐先手写 `run_simple_agent`（本节 (九)），理解"任务分类 → 工具选择 → 结果整合"的完整流程。LangChain Agent 框架作为进阶方案，可在系统集成阶段（第 6 章）或学有余力时尝试。两种方式使用的工具定义（5.2）和 LLM 客户端（4.5/6.1）完全一致。
