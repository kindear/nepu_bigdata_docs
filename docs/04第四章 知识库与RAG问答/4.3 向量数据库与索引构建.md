...
4.3 的核心是 FAISS 索引构建。检索函数的完整实现（问题编码 → FAISS 查询 → 结果映射 → 字段提取）见 **4.4（三）`retrieve_chunks`**，该函数还包含 `rank` 排序和完整的来源字段映射。本节 (十) 已展示 `index.search()` 的基本用法，可直接用于索引构建后的快速验证。

```python
# 快速验证用：直接使用 FAISS API
scores, indices = index.search(query_vector, top_k=5)

for rank, chunk_index in enumerate(indices[0], start=1):
    if chunk_index >= 0:
        print(f"排名{rank}：{chunks[chunk_index]['chunk_text'][:50]}...")
```

完整检索实现（含过滤、去重、重排序）见 4.4。
...
> **拓展：LangChain FAISS 封装**：LangChain 提供了 `langchain_community.vectorstores.FAISS`，可以替代本节手写的 `faiss.IndexFlatIP` 创建和检索流程。其 `from_documents` 和 `similarity_search` 方法能自动处理向量添加和查询，并与 4.2 的 `Document` 对象直接衔接。但封装层次较深，不利于理解 FAISS 索引构建原理。课程基础项目推荐使用本节原生 `faiss-cpu` 方式，学有余力时再了解 LangChain FAISS 封装。
>
> LangChain 还提供了 `BaseRetriever` 统一检索接口，可将 FAISS 检索、结果过滤和重排序组合为一条检索链，详见 4.4 末尾的拓展提示。
