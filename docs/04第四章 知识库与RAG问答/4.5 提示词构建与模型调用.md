...
> **整合指引：使用 LangChain ChatPromptTemplate**：本节使用手写字典构建 `messages` 列表，再通过 `call_llm` 转换为 LangChain 的 `SystemMessage`/`HumanMessage`。系统集成阶段（第 6 章）将使用 LangChain 的 `ChatPromptTemplate` 和 `StrOutputParser` 管道（`prompt | llm | StrOutputParser()`）来替代这一手动流程，代码更简洁且支持变量插值。详见 **6.1（六）使用 LangChain 构建提示词**。两种方式的核心逻辑一致：系统提示词 + 知识上下文 + 用户问题 → DeepSeek 生成回答。
