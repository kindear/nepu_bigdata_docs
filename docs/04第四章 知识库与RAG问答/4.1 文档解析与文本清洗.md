...
文档解析程序从 PySpark 输出的 Parquet 文件中获取已清洗的文档列表。这些数据已经过去重、字段标准化和质量标记，状态为 `valid` 的记录才能进入知识库。

```python
# 读取 PySpark 清洗后的文档列表
pages_df = spark.read.parquet("data/cleaned/pages")
valid_pages = pages_df.filter(F.col("data_status") == "valid")
```

网页和附件的读取方式需要区分：

- 网页正文优先使用 Parquet 中已经清洗后的 `content` 字段；
- 如需重新解析网页 HTML，可根据 `html_object_key` 从 S3 读取原始 HTML；
- PDF、Word、Excel 等附件不直接来自 `pages` Parquet，应从 MySQL 的 `attachment` 表或附件 JSONL 中读取 `attachment_id`、`document_id`、`file_name` 和 `object_key`；
- 附件解析时再根据附件记录中的 `object_key` 从 S3 读取原始文件。

这样可以避免把网页 HTML 路径和附件对象路径混用。
...
解析程序不依赖固定的本地文件路径，便于在不同计算机或服务器中运行。
